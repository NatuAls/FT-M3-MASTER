...
function problemB () {
  ...
  // promise version
  let allStanzas = filenames.map(e => {
    return promisifiedReadFile(e).then(stanza => blue(stanza));
  });
  Promise.all(allStanzas).then(() => console.log('done'));
}
...
function problemC () {
  ...
  // promise version
  let allStanzas = filenames.map(e => {
    return promisifiedReadFile(e);
  });
  Promise.all(allStanzas).then(values => values.forEach(e => {
    blue(e);
