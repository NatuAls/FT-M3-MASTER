...
async function problemA () {
  ...
  // AsyncAwait version
  async function asyncFunc(filename){
    blue(await promisifiedReadFile(filename));
  }
  await Promise.all([asyncFunc('poem-two/stanza-01.txt'), asyncFunc('poem-two/stanza-02.txt')])
  console.log('done');
}
...
async function problemB () {
  ...
  await Promise.all(filenames.map(e => asyncFunc(e)))
  console.log('done');
}
...
async function problemC () {
  ...
  // AsyncAwait version
  let allFilenames = await Promise.all(filenames.map(filename => promisifiedReadFile(filename)));
  await allFilenames.forEach(filename => blue(filename));
  console.log('done');
}
